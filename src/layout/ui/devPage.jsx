  ...
  return (
    <HtmlShell>
      <main className="max-w-5xl p-8">
        <header className="italic subpixel-antialiased">
          Index of Directory {props.directory}/...
        </header>
        <ul className="grid grid-cols-4 gap-3 p-8 list-none text-sm">
          {props.files.map((file) => {
            return (
              <li key={file.name} className=" m-4">
                <a href={file.href} className="flex flex-row text-left">
                  <span>
                    {file.type === "folder" ? <FolderSVG /> : <FileSVG />}
                  </span>
